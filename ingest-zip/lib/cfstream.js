...
export default function(stackName, options) {
    if (!options) options = {};
    const cfn = new CloudFormation.CloudFormationClient(options);
    ...
    async function describeEvents(nextToken) {
        ...
        // not unique by name.
        try {
            const data = await cfn.send(new CloudFormation.DescribeStackEventsCommand({
                StackName: stackId,
                NextToken: nextToken
            }));
            describing = false;
    ...
    async function describeStack() {
        ...
        describing = true;
        try {
            const data = await cfn.send(new CloudFormation.DescribeStacksCommand({
                StackName: stackId
            }));
            describing = false;
