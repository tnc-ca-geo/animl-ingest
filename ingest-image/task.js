...
export default class Task {
  constructor(stage = 'dev') {
    ...
    this.SUPPORTED_FILE_TYPES = ['.jpg', '.jpeg', '.png']; // If this is changed also update ingest-zip
  ...
  async hash(img_name) {
    return createHash('md5')
      .update(await fsp.readFile(path.resolve(this.tmp_dir, img_name)))
      .digest('hex');
  }
...
async function fetcher(url, body) {
  console.log('Posting to API', JSON.stringify(body));
